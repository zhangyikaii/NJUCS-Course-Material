...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    UnknownCity = make_city("NamelessShit", lat, lon)
    if distance(city1, UnknownCity) < distance(city2, UnknownCity):
...
def add_trees(t1, t2):
    ...
    while i < len(b1) or j < len(b2):
        if i < len(b1) and j < len(b2):
            branch.append(add_trees(branches(t1)[i], branches(t2)[j]))
            i += 1
            j += 1
        elif i < len(b1) and j == len(b2):
            branch.append(add_trees(b1[i], tree(0)))
            i += 1
        elif i == len(b1) and j < len(b2):
            branch.append(add_trees(tree(0), b2[j]))
            j += 1
