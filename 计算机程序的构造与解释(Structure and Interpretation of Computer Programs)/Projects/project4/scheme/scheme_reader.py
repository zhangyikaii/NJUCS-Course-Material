...
def isTail(expr):
    ...
    return isCall(last) and last.first == name
    """if not isinstance(last,Pair):
        return False
    elif last.first == "if":
        return expr is last.rest.rest.first or expr is last.rest.rest.rest.first
    elif last.first == 'and' or last.first == 'or' or last.first == 'begin' or last.first == 'let':
        return expr is pureLast(last)"""
