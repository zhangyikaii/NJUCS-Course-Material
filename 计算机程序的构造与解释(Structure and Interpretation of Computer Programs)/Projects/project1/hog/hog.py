...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        result += temp
        num_rolls -= 1
    return f if f else result
    # END PROBLEM 1


def digits(anInteger):
    """我干的"""
    aString = str(anInteger)
    returnIntArray = [0] * len(aString)
    i = 0
    while i < len(aString):
        returnIntArray[i] = int(aString[i])
        i += 1
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    result = 0
    preDone = digits(score ** 3)
    i = 0
    while i < len(preDone):
        result += preDone[i] * ((-1) ** i)
        i += 1
    return abs(result) + 1
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    condition = digits(3 ** (player_score + opponent_score))
    return condition[0] == condition[len(condition) - 1]
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            fStrat = strategy0(score0, score1)
            currentGet0 = take_turn(fStrat, score1, dice)
            if feral_hogs:
                if abs(fStrat - lastGet0) == 2:
                    score0 += 3
            lastGet0 = currentGet0
            score0 += currentGet0
        else:
            sStrat = strategy1(score1, score0)
            currentGet1 = take_turn(sStrat, score0, dice)
            if feral_hogs:
                if abs(sStrat - lastGet1) == 2:
                    score1 += 3
            lastGet1 = currentGet1
            score1 += currentGet1
        who = other(who)
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    # BEGIN PROBLEM 7
    def Monkey1(score0, score1):
        if score0 - prev_score > prev_high:
            print(str(score0 - prev_score) + " point(s)! That's the biggest gain yet for Player 0")
            return announce_highest(who, score0 - prev_score, score0)
        else:
            return announce_highest(who, prev_high, score0)

    def Monkey2(score0, score1):
        if score1 - prev_score > prev_high:
            print(str(score1 - prev_score) + " point(s)! That's the biggest gain yet for Player 1")
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    expect = free_bacon(opponent_score)
    originS = score
    score += expect
    if expect >= margin:
        return 0
